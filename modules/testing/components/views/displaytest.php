...
<?php
    ...
    foreach ($test['questions'] as $id_question=>$question) {
        ...
        <div class="panel panel-default">
            <div class="panel-heading" style="background-color: #72b6e5; font-style: italic;"><?= $question['title']?></div>
            ...
            echo $form->field($model,'answers['.$id_question.'][]')->radioList(ArrayHelper::getColumn($question['answers'],'text'),
                [
                    ...
                    'item' => function ($index, $label, $name, $checked, $value) {
                        return '<div class="radio"><label style="font-weight: bold" ' . ($checked ? ' active' : '') . '">' .
        ...
        else
        {
            echo $form->field($model,'answers['.$id_question.'][]')->checkboxList(ArrayHelper::getColumn($question['answers'],'text'),
                [
                    'unselect' => null,
                    'item' => function ($index, $label, $name, $checked, $value) {
                        return '<div class="checkbox"><label style="font-weight: bold" ' . ($checked ? ' active' : '') . '">' .
